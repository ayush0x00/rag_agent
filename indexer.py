...
class Chunker:
    def __init(self, chunk_size: int = 3000):
        self.chunk_size = chunk_size

    def chunk(self, text: str, file_path: str = None):
        return [
            {
                "id": f"chunk_{i:03d}",
    ...
    def chunk_file(self, file_path: str):
        with open(file_path, "r", encoding="utf-8") as f:
            text = f.read()
        return self.chunk(text, file_path)
...
class Indexer:
    ...
    def search_index(self, query_vector, top_k: int = 5):
        query = (
            Query(f"(*) => [KNN {top_k} @embeddings $query_vector AS vector_store]")
            .sort_by("vector_score")
            .return_fields(
                "id", "text", "source_file", "chunk_index", "timestamp", "vector_store"
            )
            .dialect(2)
        )
        ...
        try:
            ...
            for chunk, embedding in zip[tuple](chunks, embeddings):
                ...
                doc_data = {
                    "id": chunk["id"],
                    "text": chunk["text"],
                    "embeddings": np.array(embedding, dtype=np.float32).tobytes(),
                    "metadata": {
